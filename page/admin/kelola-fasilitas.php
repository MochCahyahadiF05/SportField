...
$page_subtitle = "Manage semua fasilitas yang tersedia";
...
try {
    $stmt = $pdo->query("SELECT * FROM fasilitas ORDER BY nama ASC");
    $fasilitas_list = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    error_log("Error loading fasilitas: " . $e->getMessage());
}
    ...
    <table style="width: 100%; border-collapse: collapse;">
        <thead>
            <tr style="border-bottom: 2px solid #e5e7eb;">
                ...
                <th style="text-align: left; padding: 12px; color: #6b7280; font-weight: 600; font-size: 14px;">Nama Fasilitas</th>
        ...
        <tbody>
            ...
            <?php else: ?>
                ...
                <tr style="border-bottom: 1px solid #e5e7eb;">
                    ...
                    <td style="padding: 12px;"><?php echo htmlspecialchars($fasilitas['nama']); ?></td>
                    <td style="padding: 12px; text-align: center;">
                        <button onclick="openEditModal(<?php echo $fasilitas['id']; ?>, '<?php echo htmlspecialchars($fasilitas['nama']); ?>')" 
                        ...
                        <button onclick="openDeleteModal(<?php echo $fasilitas['id']; ?>, '<?php echo htmlspecialchars($fasilitas['nama']); ?>')" 
...
<div id="addModal" class="modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 50; align-items: center; justify-content: center;">
    <div style="background: white; border-radius: 8px; width: 100%; max-width: 500px; padding: 24px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; border-bottom: 1px solid #e5e7eb; padding-bottom: 16px;">
            <h3 style="font-size: 20px; font-weight: bold; color: #111827; margin: 0;">Tambah Fasilitas</h3>
        ...
        <form id="addForm" onsubmit="handleAdd(event)">
            <div style="margin-bottom: 16px;">
                <label style="display: block; margin-bottom: 8px; font-weight: 500;">Nama Fasilitas *</label>
...
<div id="deleteModal" class="modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 50; align-items: center; justify-content: center;">
    <div style="background: white; border-radius: 8px; width: 100%; max-width: 500px; padding: 24px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; border-bottom: 1px solid #e5e7eb; padding-bottom: 16px;">
            <h3 style="font-size: 20px; font-weight: bold; color: #111827; margin: 0;">Hapus Fasilitas?</h3>
            <button onclick="closeDeleteModal()" style="background: none; border: none; font-size: 32px; cursor: pointer; color: #6b7280; padding: 0; line-height: 1;">&times;</button>
        </div>
...
<div id="successModal" class="modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 50; align-items: center; justify-content: center;">
    <div style="background: white; border-radius: 8px; width: 100%; max-width: 500px; padding: 24px; text-align: center;">
        ...
        <p id="successMessage" style="color: #6b7280; margin-bottom: 24px; font-size: 14px;">Fasilitas berhasil ditambahkan</p>
...
function handleAdd(event) {
    ...
    formData.append('table', 'fasilitas');
    ...
    .then(data => {
        if (data.success) {
            showSuccessModal('Fasilitas Ditambahkan', 'Fasilitas berhasil ditambahkan. Halaman akan di-reload dalam 2 detik.');
            closeAddModal();
...
function handleEdit(event) {
    ...
    .then(data => {
        if (data.success) {
            showSuccessModal('Fasilitas Diperbarui', 'Fasilitas berhasil diperbarui. Halaman akan di-reload dalam 2 detik.');
            closeEditModal();
            setTimeout(() => location.reload(), 2000);
        } else {
...
function confirmDelete() {
    ...
    fetch('admin-table-handler.php', {
        ...
        body: 'action=delete&id=' + deleteId + '&table=fasilitas'
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            showSuccessModal('Fasilitas Dihapus', 'Fasilitas berhasil dihapus. Halaman akan di-reload dalam 2 detik.');
            closeDeleteModal();
            setTimeout(() => location.reload(), 2000);
        } else {
